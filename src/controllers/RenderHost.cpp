...
RenderHost::~RenderHost() noexcept
{
...
void RenderHost::render(
        const PanelHost& panel_host
)
{
...
bool RenderHost::wait_for_frame_trigger() const noexcept
{
...
void RenderHost::process_instance_events() const
{
    instance.ProcessEvents();
}

GLFWwindow* RenderHost::create_window(
        const int width,
        const int height
)
{
...
void RenderHost::configure_surface(
...
wgpu::Future RenderHost::request_adapter() noexcept
{
...
wgpu::Future RenderHost::request_device() noexcept
{
...
void RenderHost::setup_dockspace()
{
    ...
    if (!dockspace_configured) {
        ...
        if (ImGui::DockBuilderGetNode(dockspace_id) == nullptr) {
            ...
            PanelHost::configure_default_dockspace(dockspace_id);
...
bool RenderHost::handle_window_resize() noexcept
{
    int fb_width = 0;
    int fb_height = 0;
